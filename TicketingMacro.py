...
macro_form = resource_path('TicketingMacro.ui')
...
class MyWindow(QtWidgets.QMainWindow, form_class):
    ...
    def initData(self):
        if not self.is_data_disabled:  # 정보 입력창 활성화 시
            ...
            self.seq = self.seq_input.text()
            self.use_random_seat = self.use_seat_select.isChecked()  # 중간(랜덤) 선택 여부
            self.is_map_separated = self.map_separate.isChecked()
            ...
            self.disableElements(self.user_id_input, self.user_password_input, self.product_code_input, self.date_edit, self.seq_input, self.map_separate,
            ...
            self.enableElements(self.user_id_input, self.user_password_input, self.product_code_input, self.date_edit, self.seq_input, self.map_separate, 
    ...
    def startLogin(self):
        ...
        if self.apply_btn.text() == '확인':  # 확인 버튼을 누르지 않았을 때
            ...
            ticketing_data_to_send = {
                ...
                'seq': self.seq, #회차
                'is_separated': self.is_map_separated, #구역 분리 여부
...
class TicketingThread(QtCore.QThread):
    def __init__(self, ticketing_data, parent=None):
        ...
        self.seq = ticketing_data['seq']
        self.is_map_separated = ticketing_data['is_separated']
    ...
    def selectSeat(self):
        ...
        # 직링 생성
        self.url = 'http://poticket.interpark.com/Book/BookSession.asp?GroupCode={}&Tiki=N&Point=N&PlayDate={}&PlaySeq={}&BizCode=&BizMemberCode='.format(self.product_code, self.date, self.seq)
        ...
        while True:
            try:
                if self.is_map_separated:
                    elements = self.driver.find_elements_by_class_name('SeatN')
                else:
            ...
            while True:
                try: #안전예매시 예외 방어
                    ...
                    if (not self.use_random_seat) and (self.seats_number == 1):
                        if self.is_map_separated:
                            self.driver.find_element_by_css_selector('#Seats').click()
                        else:
