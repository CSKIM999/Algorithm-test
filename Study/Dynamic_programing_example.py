...
###############################################################################################################
'''
Given ) 두개의 문자열 A 와 B 가 주어질 때 세가지 기능을 사용하여 문자열 A 를 문자열 B 로 만들고자 한다.
        세가지 기능은 1. 삽입(특정 위치에 문자 삽입) 2. 삭제(특정 위치 문자 삭제) 3. 교체(특정 위치 문자 교체)
        각 기능은 한번 사용할 때 마다 편집 거리가 1 증가한다. 문자열 A 에서 B 로 바꾸는데 최소 편집 거리를
        계산하는 프로그램을 작성하라.
Input ) 두 문자열 A 와 B 가 한줄에 하나씩 주어진다
        각 문자열은 영문 알파벳으로만 주어진다. ( 1 <= len(A),len(B) <= 5,000 )
Output) 최소 편집 거리를 출력하라
'''
...
'''
2차원 행렬을 이용하라고 하는데 잘 모르겠음 다음기회에
'''